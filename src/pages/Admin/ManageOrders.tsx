...
const ManageOrders = () => {
  ...
  const handleDateChange = (orderId: string, selectDate: Date) => {
    ...
    // handle logic to filter or update orders based on selected date
  };
    ...
    <div className="2xl:mt-24">
      ...
      <div className="container px-4 2xl:px-0 mx-auto py-4">
        ...
        {(orders?.length ?? 0) > 0 ? (
                ...
                <tbody>
                  {orders?.map((order, i: number) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3">{order.transaction?.method}</td>
                      <td className="px-6 py-3">
                        <button
          ...
          <>{/* <NotFound text={"orders are not available"} /> */}</>
        )}
      </div>
    </div>
  );
